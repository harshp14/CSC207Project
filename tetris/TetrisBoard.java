...
public class TetrisBoard implements Serializable{
    ...
    public int placementHeight(TetrisPiece piece, int x) {
        int[] lowestYVals = piece.getLowestYVals(), heights = new int[lowestYVals.length];
        int max = 0;

        for (int i = 0; i < heights.length; i++) {heights[i] = this.getColumnHeight(x + i) - lowestYVals[i];}
        for (int height : heights) {if (max < height){max = height;}}
        return max;
    }
    ...
    public int placePiece(TetrisPiece piece, int x, int y) {
        backupGrid();
        committed = false;
        if (piece.getWidth() + x > this.getWidth() || piece.getHeight() + y > this.getHeight() || x < 0 || y < 0) {return ADD_OUT_BOUNDS;}

        //Checks if all spots used by the piece are empty, exits function if not
        for (TetrisPoint point : piece.getBody()) {if (getGrid(point.x + x, point.y + y)) {return ADD_BAD;}}

        //Fills all used positions in the tetris board, since we are given that all spots are empty
        for (TetrisPoint point : piece.getBody()) {
            this.tetrisGrid[point.x + x][point.y + y] = true;
            //Only updates colCounts if it is smaller then the current position
            if (this.colCounts[x + point.x] < point.y + y + 1) {this.colCounts[x + point.x] = point.y + y + 1;}
            this.rowCounts[y + point.y] += 1;
        }

        for (int i = y; i < (y + piece.getHeight()); i++) {if (this.getRowWidth(y) == this.width){return ADD_ROW_FILLED;}}
        return ADD_OK;
    }
    ...
    public int clearRows() {
        for (int i = 0; i < getHeight() - 1; i++) {
            if (getRowWidth(i) == getWidth()) {
                //Update rowCounts and move all blocks down by 1
                for (int j = i; j < getHeight() - 1; j++) {for (int w = 0; w < getWidth(); w++) {tetrisGrid[w][j] = getGrid(w, j + 1);}rowCounts[j] = rowCounts[j + 1];}
                //Update colCounts
                for (int k = 0; k < colCounts.length; k++) {colCounts[k]--;}
                return 1 + clearRows();
            }
        }
        return 0;
